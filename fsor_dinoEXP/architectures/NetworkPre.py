...
from models_encoders.encoders.resnet12 import resnet12
...
class FeatureNet(nn.Module):
    def __init__(self,args,restype,n_class,param_seman, oslo_paras):
        ...
        if self.restype == 'ResNet12_dino':
            self.feature = ptcv_get_model("resnet12", pretrained=False)
            params = torch.load('models/resnet12_dino.pth')
            for name, param in params['teacher'].items():
                name = name[16:]
                if name in self.feature.state_dict() and param.shape == self.feature.state_dict()[name].shape:
                    self.feature.state_dict()[name].copy_(param)
            self.feature.output = nn.Linear(in_features=512, out_features=640, bias=True)
            self.feat_dim = 640
        if self.restype == 'ResNet50_dino':
            self.feature = torch.hub.load('facebookresearch/dino:main', 'dino_resnet50')
            self.feature.fc = nn.Linear(in_features=2048, out_features=384)
            self.feat_dim = 384
        if self.restype == 'vit-small_dino':
            self.feature = torch.hub.load('facebookresearch/dino:main', 'dino_vits16')
            self.feat_dim = 384
        ...
        self.cls_classifier = Classifier(args, self.feat_dim, param_seman, oslo_paras, args.train_weight_base)
    ...
    def open_forward(self, the_input, labels, conj_ids, base_ids, test):
        ...
        support_feat,query_feat,supopen_feat,openset_feat = torch.split(combined_feat.contiguous().view(ne,-1,self.feat_dim),the_sizes,dim=1)
        ...
        test_cosine_scores, supp_protos, fakeclass_protos, loss_ce, loss_op, loss_oslo = self.task_proto((support_feat,query_feat,openset_feat), (supp_idx,base_ids), cls_label, query_label, test)
        ...
        _, supp_protos_aug, fakeclass_protos_aug, loss_ce_aug, loss_op_aug, loss_oslo_aug = self.task_proto((supopen_feat,openset_feat,query_feat), (open_idx,base_ids), cls_label, query_label, test)
        ...
        loss = (loss_ce + loss_ce_aug, loss_op + loss_op_aug)
    ...
    def task_proto(self, features, cls_ids, cls_label, query_label, test=False):
        ...
        if self.args.protonet:
            highest, _ = torch.max(cls_scores, dim=-1, keepdim=True)
            cls_scores = torch.cat([cls_scores,highest], dim=-1)
            test_cosine_scores = (cls_scores[:, :75, :], cls_scores[:, 75:, :])
        ...
        loss_ce = F.cross_entropy(cls_scores, query_label)
        return test_cosine_scores, supp_protos, fakeclass_protos, loss_ce, fakeunit_loss, loss_oslo
